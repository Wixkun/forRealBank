...
export function BalanceCard({ label, amount, growthText }: BalanceCardProps) {
  ...
  return (
    <div
      className={`rounded-3xl p-8 mb-6 relative overflow-hidden shadow-2xl border ${
        currentTheme === 'dark'
    ...
    >
      ...
      <div className="relative">
        <p
          className={`text-base mb-2 ${currentTheme === 'dark' ? 'text-gray-400' : 'text-white/90'}`}
        >
          {label}
        </p>
        <p
        ...
        {growthText && (
          <div
            className={`flex items-center gap-2 text-base backdrop-blur-sm rounded-full px-3 py-1 w-fit border mt-4 ${
              currentTheme === 'dark'
