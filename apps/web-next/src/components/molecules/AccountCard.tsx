...
export function AccountCard({
  ...
  return (
    <Link
      href={href}
      className={`block backdrop-blur-sm hover:shadow-xl rounded-2xl p-5 transition cursor-pointer group border ${
        currentTheme === 'dark'
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <div
              className={`w-3 h-3 rounded-full shadow-lg ${type === 'checking' ? 'bg-gradient-to-r from-teal-400 to-cyan-400' : 'bg-gradient-to-r from-gray-400 to-gray-500'}`}
            ></div>
            <h4
              className={`font-medium text-base ${currentTheme === 'dark' ? 'text-white' : 'text-gray-900'}`}
            >
              {name}
            </h4>
          </div>
          {iban && (
            <p
              className={`text-sm font-mono ${currentTheme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}
            >
              {iban}
            </p>
          )}
