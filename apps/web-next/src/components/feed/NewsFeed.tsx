...
type NewsStatus = 'SECURITY' | 'TRANSACTIONS' | 'PAYMENTS' | 'ACCOUNT_UPDATES' | 'SYSTEM' | 'INFORMATION';
...
const STATUS_CONFIG: Record<NewsStatus, { label: string; bg: string; color: string; icon: JSX.Element }> = {
  SECURITY: {
    label: 'Security',
    bg: 'bg-rose-500/15',
    color: '#f43f5e',
...
function DraggableNewsItem({ item, onArchive, onDelete, isNew }: DraggableNewsItemProps) {
  ...
  return (
    ...
    >
      ...
      >
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#f87171" strokeWidth="2.5" strokeLinecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/></svg>
      ...
      >
        ...
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2.5" strokeLinecap="round"><path d="M21 8v13H3V8"/><path d="M1 3h22v5H1z"/><path d="M10 12h4"/></svg>
      </div>
...
export default function NewsFeed({ apiUrl = '/api', userRoles = null, userId = null }: NewsFeedProps) {
  ...
  const canPublish = Array.isArray(userRoles) &&
    (userRoles.includes('ADVISOR') || userRoles.includes('DIRECTOR'));
    ...
    <div className="p-5">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="text-cyan-400 text-[11px] hover:text-cyan-300 transition">
            Voir tout
          </button>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {displayedNews.length === 0 ? (
          ...
          archived.map((item) => {
            const cfg = STATUS_CONFIG[item.status] ?? STATUS_CONFIG.INFORMATION;
            return (
              <div key={item.id} className="flex items-start gap-3 p-3 bg-[#1a1d24]/50 rounded-xl border border-white/4">
